...
class FormulaireClientBase(QWidget):
    ...
    def _collecter_donnees(self) -> dict:
        ...
        donnees = {
            ...
            "sexe": self.combo_sexe.currentText() if hasattr(self, "combo_sexe") else "",
            "email": email,
    ...
    def charger_client(self, client_id: int):
        ...
        # Sexe
        if hasattr(self, "combo_sexe") and client.get("sexe"):
            idx = self.combo_sexe.findText(client["sexe"])
            if idx >= 0:
                self.combo_sexe.setCurrentIndex(idx)
    ...
    def _construire_section_infos_generales(self):
        ...
        row += 1
        # --- Ligne 2 : Sexe | Date naissance ---
        lbl_sexe = QLabel("Sexe :")
        lbl_sexe.setFont(font_label)
        lbl_sexe.setStyleSheet(style_label)
        grid.addWidget(lbl_sexe, row, 0)

        self.combo_sexe = QComboBox()
        self.combo_sexe.setFont(font_corps)
        self.combo_sexe.addItems(["", "M", "F"])
        self.combo_sexe.setStyleSheet(
            "QComboBox { border: 1px solid #E0E0E0; border-radius: 6px; "
            "padding: 8px 12px; background-color: #FAFAFA; }"
            "QComboBox:focus { border: 2px solid #2196F3; background-color: #FFFFFF; }"
        )
        self.combo_sexe.setMinimumHeight(38)
        grid.addWidget(self.combo_sexe, row, 1)

        lbl_naissance = QLabel("Date de naissance :")
        lbl_naissance.setFont(font_label)
        lbl_naissance.setStyleSheet(style_label)
        grid.addWidget(lbl_naissance, row, 2)
        ...
        grid.addWidget(self.input_date_naissance, row, 3)

...
from utils.styles import style_toggle, style_input, Couleurs
from views.formulaire_client_base import FormulaireClientBase
from views.profil_client_widget import ProfilClientWidget
...
class ClientView(QWidget):
    ...
    def _construire_ui(self):
        ...
        self.btn_toggle_liste.clicked.connect(
            lambda: self._changer_page_client(self.PAGE_LISTE)
        )

        self.btn_toggle_creation = QPushButton("Nouveau client")
        self.btn_toggle_creation.setCursor(Qt.CursorShape.PointingHandCursor)
        self.btn_toggle_creation.clicked.connect(
            lambda: self._changer_page_client(self.PAGE_CREATION)
        )
    ...
    def _changer_page_client(self, index: int):
        ...
        self.btn_toggle_creation.setStyleSheet(
            style_toggle(index == self.PAGE_CREATION)
        )
    ...
    def _creer_page_liste_clients(self) -> QWidget:
        ...
        layout_profil_wrapper.setSpacing(8)
        ...
        # Scroll + profil
        scroll_profil = _QScrollArea()
        scroll_profil.setWidgetResizable(True)
        scroll_profil.setFrameShape(QFrame.Shape.NoFrame)

        self.profil_client_widget = ProfilClientWidget()
        scroll_profil.setWidget(self.profil_client_widget)

        layout_profil_wrapper.addWidget(scroll_profil, stretch=1)

...
class EmailingView(QWidget):
    ...
    def _creer_section_destinataires(self) -> QGroupBox:
        ...
        self.list_resultats_client.itemClicked.connect(self._selectionner_client)
    ...
    def _appliquer_filtres(self):
        ...
        try:
            ...
            interets = (
                [i.strip() for i in interets_text.split(",") if i.strip()]
                if interets_text
                else None
            )
    ...
    def _creer_section_contenu(self) -> QGroupBox:
        ...
        self.input_objet.setPlaceholderText("Ex: Nouvelle promotion sur nos produits")
        ...
        btn_centre.clicked.connect(
            lambda: self.text_message.setAlignment(Qt.AlignmentFlag.AlignCenter)
        )
        ...
        btn_droite.clicked.connect(
            lambda: self.text_message.setAlignment(Qt.AlignmentFlag.AlignRight)
        )
    ...
    def _creer_pj_rectangle(self, chemin: str) -> QFrame:
        ...
        lbl.setStyleSheet(
            f"font-size: 11pt; color: {Couleurs.PRIMAIRE_FONCE}; border: none;"
        )
        h.addWidget(lbl)
    ...
    def _collecter_destinataires_emails(self) -> list[str]:
        """Collecte les adresses email des destinataires selectionnes."""
        emails = []
        for i in range(self.list_destinataires.count()):
            item = self.list_destinataires.item(i)
            texte = item.text()
            # Format: "Prenom Nom - email@example.com"
            if " - " in texte:
                email_part = texte.split(" - ")[-1].strip()
                if "@" in email_part:
                    emails.append(email_part)
        return emails

    def _collecter_type_envoi(self) -> str:
        """Retourne le type d'envoi selectionne."""
        idx = self.combo_type.currentIndex()
        if idx == 0:
            return "client_unique"
        elif idx == 1:
            return "selection"
        return "tous_clients"

    def _collecter_filtres(self) -> str:
        """Retourne les filtres sous forme de string (IDs separes par virgules)."""
        ids = []
        for i in range(self.list_destinataires.count()):
            item = self.list_destinataires.item(i)
    ...
    def _enregistrer_template(self):
        ...
        nom, ok = QInputDialog.getText(self, "Nom du template", "Nom :")
        if ok and nom:
            objet = self.input_objet.text().strip()
            contenu_html = self.text_message.toHtml()
            contenu_texte = self.text_message.toPlainText()
            pj = ",".join(self.pj_fichiers) if self.pj_fichiers else ""
            try:
    ...
    def _programmer_envoi(self):
        """Ouvre le dialog de programmation et programme l'envoi."""
        ...
        if self.list_destinataires.count() == 0:
            QMessageBox.warning(
                self, "Erreur", "Veuillez selectionner des destinataires."
            )
            return
        ...
        layout = QVBoxLayout()
        layout.addWidget(QLabel("Choisissez la date et l'heure d'envoi :"))
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            try:
                ...
                date_prog = dt.toString("yyyy-MM-dd")
                heure_prog = dt.toString("HH:mm")
                type_envoi = self._collecter_type_envoi()
                filtres = self._collecter_filtres()

                prog_id = self._scheduler.programmer_envoi(
                    mail_id=mail_id,
                ...
                if prog_id > 0:
                    dt_str = dt.toString("dd/MM/yyyy a HH:mm")
                    QMessageBox.information(
                        self,
                        "Envoi programme",
                        f"Email programme pour le {dt_str}.",
                    )
                else:
                    QMessageBox.warning(
                        self, "Erreur", "Erreur lors de la programmation."
                    )
            except Exception as e:
                QMessageBox.critical(self, "Erreur", f"Erreur : {e}")
    ...
    def _enregistrer_brouillon(self):
        ...
        contenu_html = self.text_message.toHtml()
        contenu_texte = self.text_message.toPlainText()
        nom = objet if objet else "Brouillon sans objet"
        pj = ",".join(self.pj_fichiers) if self.pj_fichiers else ""
        try:
            from viewmodels.mails_vm import MailsViewModel
    ...
    def _envoyer_email(self):
        ...
        if self.list_destinataires.count() == 0:
            QMessageBox.warning(
                self, "Erreur", "Veuillez selectionner au moins un destinataire."
            )
            return
        ...
        contenu_html = self.text_message.toHtml()
        contenu_texte = self.text_message.toPlainText()
        destinataires = self._collecter_destinataires_emails()

        if not destinataires:
            QMessageBox.warning(self, "Erreur", "Aucune adresse email valide trouvee.")
            return

        type_envoi = self._collecter_type_envoi()

        result = self._sender.envoyer(
            destinataires=destinataires,
            objet=objet,
            contenu_html=contenu_html,
            contenu_texte=contenu_texte,
            pieces_jointes=self.pj_fichiers if self.pj_fichiers else None,
        )

        # Enregistrer dans l'historique
        self._sender.enregistrer_historique(
            objet=objet,
        ...
        if result["ok"]:
            nb = result.get("nb_envoyes", len(destinataires))
            QMessageBox.information(
                self,
                "Email envoye",
                f"Email envoye avec succes a {nb} destinataire(s).",
            )
            self.email_envoye.emit()
            self._reinitialiser()
        else:
            QMessageBox.critical(
                self,
                "Erreur d'envoi",
                f"L'envoi a echoue :\n{result.get('erreur', 'Erreur inconnue')}",
            )

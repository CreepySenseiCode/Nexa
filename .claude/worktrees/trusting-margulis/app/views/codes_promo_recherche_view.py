...
class CodesPromoRechercheView(QWidget):
    ...
    def _verifier_code(self):
        """Verifie le code saisi et affiche le resultat."""
        ...
        self._afficher_resultat_code(resultat, message, type_erreur)

    def _afficher_resultat_code(self, resultat, message, type_erreur):
        """Affiche le resultat de verification avec box coloree selon le type d'erreur."""
        ...
        if type_erreur is None:
            ...
            self._afficher_code_valide(frame_layout, titre_label, resultat)
        else:
            self._afficher_code_erreur(result_frame, frame_layout, titre_label, message_label, type_erreur)

        result_frame.setLayout(frame_layout)
        self.result_layout.addWidget(result_frame)
        self.result_widget.setVisible(True)

    def _afficher_code_valide(self, frame_layout, titre_label, resultat):
        """Affiche les details d'un code promo valide."""
    ...
    def _afficher_code_erreur(self, result_frame, frame_layout, titre_label, message_label, type_erreur):
        """Affiche un message d'erreur colore selon le type."""
        STYLES = {
            "EXPIRE": (
                "\U0001F7E0 Code EXPIR\u00c9", "#F57C00",
                "#FFF3E0", "#FF9800",
            ),
            "EPUISE": (
                "\U0001F534 Code \u00c9PUIS\u00c9", "#D32F2F",
                "#FFEBEE", "#F44336",
            ),
            "INEXISTANT": (
                "\u274C Code INEXISTANT", "#D32F2F",
                "#FFEBEE", "#F44336",
            ),
            "CLIENT_REQUIS": (
                "\U0001F464 Client requis", "#0277BD",
                "#E1F5FE", "#03A9F4",
            ),
        }

        titre, couleur, bg, bordure = STYLES.get(
            type_erreur,
            ("\u26A0\uFE0F Code INVALIDE", "#616161", "#F5F5F5", "#9E9E9E"),
        )

        result_frame.setStyleSheet(
            f"QFrame {{ background-color: {bg}; border: 3px solid {bordure}; "
            "border-radius: 15px; padding: 20px; }"
        )
        titre_label.setText(titre)
        titre_label.setStyleSheet(f"color: {couleur}; border: none;")
        message_label.setStyleSheet(
            f"font-size: 12pt; color: {couleur}; border: none; padding: 10px;"
        )
        frame_layout.addWidget(titre_label)
